...
const WorkshopBackdrop: React.FC<{ palace?: boolean }> = ({ palace }) => {
  return (
    <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden select-none">
      <div className={`absolute inset-0 transition-colors duration-1000 ${palace ? 'bg-[#4a0404]' : 'bg-[#0f172a]'}`} />
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-black/40 to-black opacity-80" />

      {/* Tang Architectural Motifs */}
      <div className="absolute -top-20 -left-20 w-80 h-80 opacity-[0.05] animate-spin-slow text-red-500">
      ...
      <div className="absolute top-0 left-1/4 w-[600px] h-[400px] bg-red-600/10 blur-[120px] rounded-full" />
      <div className="absolute bottom-0 right-1/4 w-[600px] h-[400px] bg-yellow-600/5 blur-[120px] rounded-full" />

      <div className={`absolute bottom-[-100px] left-1/2 -translate-x-1/2 w-[120%] h-[400px] bg-gradient-to-t via-red-600/10 to-transparent blur-3xl opacity-60 animate-furnace-glow ${palace ? 'from-yellow-900/40' : 'from-red-900/40'}`} />
      
      <div className="absolute inset-0">
        {Array.from({ length: 15 }).map((_, i) => (
          <div
            key={i}
            className={`absolute w-1 h-1 rounded-full blur-[1px] animate-[float_10s_ease-in-out_infinite] ${palace ? 'bg-yellow-400/40' : 'bg-red-400/30'}`}
            style={{
...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const isPalace = gameState.totalCatches >= 250;
